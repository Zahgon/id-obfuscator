...
public class AlphabetCodec implements IntegerCodec, LongCodec {
    ...
    private long decodeStringAsLong(final String string, final int maximumStringLength) {
        if (string.length() > maximumStringLength) {
            throw new IllegalArgumentException(String.format("String \"%s\" is too long to represent a valid integer in this codec's alphabet.", string));
        }
        ...
        for (final char c : chars) {
            try {
                ...
                if (characterValue < 0) {
                    throw new IllegalArgumentException(String.format("Could not decode \"%s\"; character '%s' not in codec alphabet.", string, c));
                }
